...
application {
    mainClass = "academy.dataflow.wind.transactions.AtLeastOnceApp"
}

// One task per program: ./gradlew runTransferSource, runAtLeastOnce, runTransactional
mapOf(
    "runTransferSource" to "TransferSource",
    "runAtLeastOnce" to "AtLeastOnceApp",
    "runTransactional" to "TransactionalApp",
).forEach { (name, main) ->
    tasks.register<JavaExec>(name) {
        group = "application"
        classpath = sourceSets["main"].runtimeClasspath
        mainClass = "academy.dataflow.wind.transactions.$main"
    }
}
